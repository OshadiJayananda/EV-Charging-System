...
function AdminDashboard() {
  ...
  const fetchActiveInactiveStations = async () => {
    const res = await getRequest<{ activeStations: number, inactiveStations: number }>("/station/count");
    if (res) {
  ...
  const fetchReservationOverview = async () => {
    const res = await getRequest<{ pendingReservations: number, approvedReservations: number, completedReservations: number , chargingReservations: number }>("/bookings/overview");
    if (res) {
  ...
  const fetchUsageAnalytics = async () => {
    const res = await getRequest<{ mostPopularStation: string, mostPopularTimeSlotStartTime: string }>("/analytics/usage");
    if (res) {
  ...
  const reservationsChartData = {
    ...
    datasets: [
      {
        label: "Reservations",
        data: [pendingReservations, approvedReservations, completedReservations, chargingReservations],
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white shadow-md rounded-lg p-6 hover:shadow-xl transition">
          <h3 className="text-lg font-semibold mb-2">Active vs Inactive Stations</h3>
          <p className="text-gray-600 mb-4">Total active and inactive stations.</p>
          <Bar data={stationsChartData} options={{ responsive: true }} height={250} />
        </div>

        {/* Reservation Overview */}
        <div className="bg-white shadow-md rounded-lg p-6 hover:shadow-xl transition">
          <h3 className="text-lg font-semibold mb-2">Reservation Overview</h3>
          <p className="text-gray-600 mb-4">Overview of pending, approved, and completed reservations.</p>
          <Pie data={reservationsChartData} options={{ responsive: true }} height={250} />
        </div>

        {/* Usage Analytics */}
        <div className="bg-white shadow-md rounded-lg p-6 hover:shadow-xl transition">
          <h3 className="text-lg font-semibold mb-2">Usage Analytics</h3>
          <p className="text-gray-600 mb-4">Stats about the most popular station and time slot.</p>
          <div>
            <p className="text-sm text-gray-600">Most Popular Station:</p>
            <p className="text-xl font-bold text-indigo-600">{mostPopularStation}</p>
          </div>
          <div className="mt-4">
            <p className="text-sm text-gray-600">Most Popular Time Slot:</p>
            <p className="text-xl font-bold text-indigo-600">{mostPopularTimeSlot}</p>
          </div>
        </div>
      </div>
    </div>
